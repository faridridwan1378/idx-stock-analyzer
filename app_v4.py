...
def main():
    ...
    with st.sidebar:
        ...
        input_method = st.radio(
            "Metode Input:",
            ["📋 Pilih dari Daftar", "✏️ Ketik Manual"],
            horizontal=True
        )
        ...
        st.markdown("---")
        period = st.selectbox("📅 Periode:", ["1mo", "3mo", "6mo", "1y", "2y", "5y"], index=3)
        ...
        <div style="text-align:center; font-size:0.8rem; color:#888;">
            ...
            <p>© 2026 Farid Ridwan</p>
            <p>📧 farid.rdwan@gmail.com</p>
        </div>
    ...
    if btn:
        if not ticker:
            st.error("❌ Masukkan kode saham terlebih dahulu!")
            return
        ...
        st.success(f"✅ Berhasil memuat **{len(df_raw)}** data untuk **{ticker}**")
        ...
        <div class="rec-box" style="background:linear-gradient(135deg,{color},{color}99);">
            ...
            <p style="font-size:1rem;opacity:0.9;">📊 {ticker}</p>
        ...
        with t3:
            ...
            st.markdown("---")
            st.subheader("🔄 Probabilitas Bersyarat")
            ...
            if 'day_of_week' in stats:
                st.markdown("---")
                st.subheader("📅 Probabilitas Per Hari")
        ...
        with t4:
            ...
            st.download_button("📥 Download CSV", df.to_csv(), f"{ticker}.csv")
    ...
    <p style='text-align:center;color:#888;font-size:0.9rem;'>
        ⚠️ <b>DISCLAIMER:</b> Ini Buat investasi Pribadi bukan Publik. Selalu lakukan riset mandiri.<br>
        © 2026 <b>Farid Ridwan</b> | 📧 farid.rdwan@gmail.com
    </p>
    """, unsafe_allow_html=True)
